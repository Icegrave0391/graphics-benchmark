...
| | `venus-windows` | Windows | venus → RADV | 实验性，见 §6 |
...
| `venus-windows` | — | Venus | DXVK→Venus（实验性，见 §6） |
...
- **Venus on Windows guest (`venus-windows`)**：Windows 端 Venus ICD 支持很不成熟，可能需要回退或标注为实验性 / 跳过。
...
- **Windows guest 帧捕获**：用 PresentMon（ETW，API 无关），与 Linux 的 MangoHud 口径需做一次校准对齐。
...
4. `virgl-linux` — 仅 GL。
5. Windows 侧 `native-windows` / `pt-windows` — 原生 DX 对照。
6. 评估 `venus-windows` 是否可行。
